...
import { garages, primaryVehicle } from '../../lib/data';
...
const CATS: { icon: IconName; label: string; href: string }[] = [
  ...
  { icon: 'bolt', label: 'Emergency', href: '/emergency' },
...
export default function Home() {
  ...
  const name = (profile?.full_name || 'Dawit').split(' ')[0];
  const v = primaryVehicle;
  const serviceLeft = Math.max(0, v.nextServiceKm - v.mileage);
  const progress = Math.min(1, v.mileage / v.nextServiceKm);
  ...
  return (
    ...
    >
      ...
      <View style={{ paddingHorizontal: 20 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <View style={{ flexDirection: 'row', gap: 8 }}>
            ...
            <Avatar name={profile?.full_name || 'Dawit Mekonnen'} size="md" ring />
          </View>
        </View>
      ...
      {/* Vehicle card (dark in both themes) */}
      <View style={{ paddingHorizontal: 20, marginTop: 18 }}>
        ...
        >
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 14, fontWeight: '700', color: colors.onEspresso }}>{v.make} {v.model} {v.year}</Text>
              <Text style={{ fontSize: 11.5, color: colors.onEspressoMuted, marginTop: 2 }}>{v.plate}</Text>
            </View>
      ...
      <View style={{ marginTop: 22 }}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 14, paddingHorizontal: 20 }}>
          {CATS.map((c) => (
            <Pressable key={c.label} onPress={() => router.push(c.href as any)} style={{ alignItems: 'center', gap: 8, width: 60 }}>
              ...
              <Text style={{ fontSize: 11, fontWeight: '600', color: colors.ink2 }}>{c.label}</Text>
            </Pressable>
          ))}
        </ScrollView>
      </View>
        ...
        {garages.map((g, i) => (
          <Card key={g.id} variant="interactive" onPress={() => router.push(`/garage/${g.id}`)} style={{ width: 210 }}>
            ...
            <View style={{ padding: 12 }}>
              ...
              <Text numberOfLines={1} style={{ fontSize: 11.5, color: colors.muted, marginTop: 4 }}>{g.area} · {g.dist}</Text>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
                <Badge label={g.dist} variant="secondary" />
                <Text style={{ fontSize: 12, fontWeight: '700', color: colors.ink }}>From {g.from} ETB</Text>
              </View>
            </View>
          </Card>
        ))}
            ...
            <View style={{ flexDirection: 'row', padding: 11, gap: 12, alignItems: 'center' }}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 5 }}>
                  ...
                  <Text style={{ fontSize: 11.5, color: colors.muted }}>· {g.dist}</Text>
                </View>
                <View style={{ marginTop: 7 }}>
                  <Badge label={`From ${formatETB(g.from)}`} variant="secondary" />
                </View>
              </View>
