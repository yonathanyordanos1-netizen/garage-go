...
// Dev-only local session (EXPO_PUBLIC_DEV_OTP=1). Lets you explore the app before
// the real SMS backend is configured. A real Supabase session always wins.
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    Promise.all([supabase.auth.getSession(), AsyncStorage.getItem(DEMO_KEY)]).then(([{ data }, demoRaw]) => {
  ...
  return (
    <AuthContext.Provider value={{ session, profile: effectiveProfile, authed: !!session || !!demo, loading, refreshProfile, demoSignIn, signOut }}>
      {children}
